...
public interface LootEntry {
    static LootEntry of(com.sigmundgranaas.forgerofabric.loot.StateFilter filter, Identifier target) {
        return new com.sigmundgranaas.forgerofabric.loot.SingleLootEntry.SingleLootEntryBuilder().weight(1).target(List.of(target)).filter(filter::filter).build();
    }
