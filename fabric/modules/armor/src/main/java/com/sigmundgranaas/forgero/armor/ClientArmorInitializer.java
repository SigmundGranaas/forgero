...
public class ClientArmorInitializer implements ClientModInitializer {
	...
	public void onInitializeClient() {
		...
		// 1. Load the same data bundle as the server-side to get component info
		...
		// 3. Run the model initializer to get texture generation tasks
		ModelInitializationResult initResult = modelInitializer.initialize(bundle.componentRegistry().all().stream().collect(Collectors.toMap(Component::id, Function.identity())), bundle.tagGraph(), new MapBackedModelRegistry());

		List<TextureGenerationTask> tasks = initResult.generationResult().textureGenerationTasks();

		// 4. Generate textures into our runtime resource pack
		generateTextures(tasks, resourceProvider);

		// 5. Generate the atlas configuration file that points to the new textures
	...
	/**
	 * Creates a `blocks.json` atlas configuration in the runtime resource pack.
	 * This file tells Minecraft to include our generated item textures in the game's atlas.
	 * We specifically exclude armor model textures, as they are not atlased.
	 *
	 * @param tasks The list of tasks from which to derive the texture identifiers.
	 */
	private void generateAtlasConfig(List<TextureGenerationTask> tasks) {
		...
		tasks.stream()
				...
				.distinct()
				// NEW: Filter out armor model textures, which should not be in an atlas.
