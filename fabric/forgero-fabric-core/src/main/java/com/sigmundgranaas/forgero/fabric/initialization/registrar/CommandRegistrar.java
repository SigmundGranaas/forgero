			...
			dispatcher.register(literal("forgero")
					.then(literal("multi_block")
							...
							.executes(context -> {
								...
								Optional<StructureTemplate> station = context.getSource().getWorld().getStructureTemplateManager().getTemplate(new Identifier("forgero:multi_block_testing"));
								...
								if (station.isPresent() && !context.getSource().getWorld().isClient) {
									...
									context.getSource().sendMessage(Text.literal("Placed Forgero multi block testing station"));
					...
					.then(literal("extended_hall")
							...
							.executes(context -> {
								...
								if (station.isPresent() && !context.getSource().getWorld().isClient) {
									...
									structureBlock.loadStructure(context.getSource().getWorld());
									structureBlock.place(context.getSource().getWorld(), true, station.get());
									context.getSource().sendMessage(Text.literal("Placed Forgero extended weapon hall"));
									context.getSource().getWorld().setBlockState(pos, initialState);
