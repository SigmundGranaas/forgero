...
public class Flexibility implements Attribute {
	...
	public Flexibility(ContainerTargetPair pair) {
		this.value = pair.container().stream().applyAttribute(pair.target(), AttributeType.FLEXIBLITY);
	}

	public static Flexibility of(PropertyContainer container) {
		var pair = ContainerTargetPair.of(container);
		return (Flexibility) AttributeCache.computeIfAbsent(pair, () -> new Flexibility(pair), KEY);
	}
	...
	public static Flexibility of(PropertyContainer container, Target target) {
