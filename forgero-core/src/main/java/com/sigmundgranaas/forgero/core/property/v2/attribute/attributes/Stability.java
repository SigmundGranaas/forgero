...
public class Stability implements Attribute {

	public static final String KEY = "STABILITY";
	private final float value;

	public Stability(ContainerTargetPair pair) {
		this.value = pair.container().stream().applyAttribute(pair.target(), AttributeType.STABILITY);
	}

	public static Stability of(PropertyContainer container) {
		var pair = ContainerTargetPair.of(container);
		return (Stability) AttributeCache.computeIfAbsent(pair, () -> new Stability(pair), KEY);
	}
	...
	public static Stability of(PropertyContainer container, Target target) {
