...
public class RecipeGametest {
	...
	// ============================================================
	// Smelting Recipe Tests (demonstrates extensibility)
	// ============================================================

	/**
	 * Verifies that smelting recipes added via SmeltingRecipeBuilder exist.
	 */
	@GameTest(templateName = FabricGameTest.EMPTY_STRUCTURE, batchId = "drp_recipes")
	public void testSmeltingRecipeExists(TestContext context) {
		RecipeManager recipeManager = context.getWorld().getRecipeManager();
		Identifier recipeId = new Identifier(TEST_NAMESPACE, "test_smelting");

		Optional<?> recipe = recipeManager.get(recipeId);
		context.assertTrue(recipe.isPresent(), "Smelting recipe should exist: " + recipeId);

		context.complete();
	}

	/**
	 * Verifies that blasting recipes added via SmeltingRecipeBuilder exist.
	 */
	@GameTest(templateName = FabricGameTest.EMPTY_STRUCTURE, batchId = "drp_recipes")
	public void testBlastingRecipeExists(TestContext context) {
		RecipeManager recipeManager = context.getWorld().getRecipeManager();
		Identifier recipeId = new Identifier(TEST_NAMESPACE, "test_blasting");

		Optional<?> recipe = recipeManager.get(recipeId);
		context.assertTrue(recipe.isPresent(), "Blasting recipe should exist: " + recipeId);

		context.complete();
	}

	/**
	 * Verifies that smelting recipes can be matched with correct input.
	 * This test verifies that the recipe is available in the RecipeManager.
	 */
	@GameTest(templateName = FabricGameTest.EMPTY_STRUCTURE, batchId = "drp_recipes")
	public void testSmeltingRecipeMatches(TestContext context) {
		RecipeManager recipeManager = context.getWorld().getRecipeManager();

		// Use get() to directly look up our smelting recipe
		Identifier recipeId = new Identifier(TEST_NAMESPACE, "test_smelting");
		Optional<?> recipe = recipeManager.get(recipeId);

		context.assertTrue(recipe.isPresent(), "Should find test_smelting recipe");
		// Verify it's actually a smelting recipe by checking it was loaded
		context.assertTrue(
				recipeManager.listAllOfType(RecipeType.SMELTING).stream()
						.anyMatch(r -> r.getId().equals(recipeId)),
				"Recipe should be registered as smelting type"
		);
	...
	/**
	 * Verifies that blasting recipes can be matched with correct input.
	 */
	@GameTest(templateName = FabricGameTest.EMPTY_STRUCTURE, batchId = "drp_recipes")
	public void testBlastingRecipeMatches(TestContext context) {
		RecipeManager recipeManager = context.getWorld().getRecipeManager();

		// Use get() to directly look up our blasting recipe
		Identifier recipeId = new Identifier(TEST_NAMESPACE, "test_blasting");
		Optional<?> recipe = recipeManager.get(recipeId);

		context.assertTrue(recipe.isPresent(), "Should find test_blasting recipe");
		// Verify it's actually a blasting recipe by checking it was loaded
		context.assertTrue(
				recipeManager.listAllOfType(RecipeType.BLASTING).stream()
						.anyMatch(r -> r.getId().equals(recipeId)),
				"Recipe should be registered as blasting type"
		);
