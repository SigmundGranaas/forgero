...
public class VanillaToolTest {
    ...
    private static ForgeroApi getApi() {
        return ForgeroInitializedCallback.getServices()
                .map(s -> s.api())
                .orElseThrow(() -> new AssertionError("ForgeroApi not available"));
    }
    ...
    public void iron_pickaxe_converts_to_component(TestContext context) {
        ...
        Optional<Component> component = getApi().converter().toComponent(pickaxe);
    ...
    public void diamond_sword_converts_to_component(TestContext context) {
        ...
        Optional<Component> component = getApi().converter().toComponent(sword);
    ...
    public void golden_axe_converts_to_component(TestContext context) {
        ...
        Optional<Component> component = getApi().converter().toComponent(axe);
    ...
    public void component_converts_back_to_itemstack(TestContext context) {
        ...
        Optional<Component> component = getApi().converter().toComponent(original);

        assertTrue(component.isPresent(), "Must convert to component");

        ItemStack converted = getApi().converter().toStack(component.get());

        assertEquals(Items.IRON_PICKAXE, converted.getItem(),
    ...
    public void diamond_pickaxe_has_binding_slot(TestContext context) {
        ...
        boolean hasBindingSlot = custom.upgrades().slots().all().stream()
                .anyMatch(slot -> slot.type().name().contains("binding") ||
    ...
    public void wooden_shovel_converts(TestContext context) {
        ...
        Optional<Component> component = getApi().converter().toComponent(shovel);
    ...
    public void stone_hoe_converts(TestContext context) {
        ...
        Optional<Component> component = getApi().converter().toComponent(hoe);
    ...
    public void all_upgrade_slots_are_empty_initially(TestContext context) {
        ...
        CustomizableComponent custom = (CustomizableComponent) component.get();
        boolean allEmpty = custom.upgrades().slots().all().stream()
                .allMatch(slot -> slot.content().isEmpty());
