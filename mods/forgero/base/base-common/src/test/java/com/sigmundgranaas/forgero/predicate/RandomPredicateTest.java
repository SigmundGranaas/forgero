...
import static com.sigmundgranaas.forgero.match.predicate.RandomPredicate.*;
import static com.sigmundgranaas.forgero.match.predicate.RandomPredicate.SeedComputations.*;
...
class RandomPredicateTest {
	...
	void testSerializationDeserialization() {
		RandomPredicate predicate = new RandomPredicate(
				0.5f, 1200, Arrays.asList(RandomPredicate.SeedSource.BLOCK_POS, RandomPredicate.SeedSource.WORLD_TIME));
		String json = RandomPredicate.CODEC.encodeStart(JsonOps.INSTANCE, predicate).result().orElseThrow().toString();
		RandomPredicate deserializedPredicate = RandomPredicate.CODEC.parse(
				JsonOps.INSTANCE, JsonParser.parseString(json)).result().orElseThrow();
	...
	public void testSeedDistribution() {
		...
		for (int count : distribution) {
			...
			assertTrue(
					Math.abs(count - expectedPerBucket) < expectedPerBucket * 0.1,
	...
	public void testSensitivityToSmallChanges() {
		...
		assertTrue(
				Long.bitCount(rotateXOROriginal ^ rotateXORChanged) > 10,
