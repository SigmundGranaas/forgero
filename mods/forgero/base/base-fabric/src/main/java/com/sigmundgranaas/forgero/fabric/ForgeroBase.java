...
import static com.sigmundgranaas.forgero.block.assemblystation.AssemblyStationBlock.ASSEMBLY_STATION_ITEM;
import static com.sigmundgranaas.forgero.block.upgradestation.UpgradeStationBlock.UPGRADE_STATION_ITEM;
...
public class ForgeroBase implements ModInitializer {
	...
	public void onInitialize() {
		invokePreInitializedEntryPoints();

		Set<String> availableDependencies = FabricLoader.getInstance().getAllMods().stream().map(ModContainer::getMetadata).map(
		...
		PipelineBuilder
				...
				.build()
				.execute();
		invokeInitializedEntryPoints(RegistryHandler.getHandler().initialize());
	}

	private void invokeInitializedEntryPoints(StateService service) {
		INITIALIZED_ENTRY_POINTS.forEach(entryPoint -> entryPoint.onInitialized(service));
	}

	private void invokePreInitializedEntryPoints() {
