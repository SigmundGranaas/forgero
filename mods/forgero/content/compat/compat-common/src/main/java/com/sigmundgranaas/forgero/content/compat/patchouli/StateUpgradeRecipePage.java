...
public class StateUpgradeRecipePage extends PageDoubleRecipeRegistry<SmithingRecipe> {
	...
	protected void drawRecipe(DrawContext context, SmithingRecipe recipe, int recipeX, int recipeY, int mouseX, int mouseY, boolean second) {
		...
		parent.renderIngredient(
				context, recipeX + 4, recipeY + 4, mouseX, mouseY, recipe.getIngredients().get(StateUpgradeRecipe.baseIndex));
		parent.renderIngredient(
				context, recipeX + 4, recipeY + 23, mouseX, mouseY, recipe.getIngredients().get(StateUpgradeRecipe.additionIndex));
