...
public class NbtToStateTest {
	...
	public static Optional<State> ingredientSupplier(String id) {
		return switch (id) {
			...
			case "forgero:iron" -> Optional.of(Upgrades.IRON);
			case "forgero:oak-binding" -> Optional.of(ToolParts.OAK_BINDING);
			case "forgero:redstone-gem" -> Optional.of(Upgrades.REDSTONE_GEM);
			...
			default -> ForgeroStateRegistry.stateFinder().find(id);
		};
	}
