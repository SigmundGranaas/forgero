...
public class MagneticHandler {
    ...
    public void pullEntities(double v, List<Entity> entities, float multiply) {
        for (Entity nearbyEntity : entities) {
            if (!nearbyEntity.getBlockPos().equals(rootEntity.getBlockPos())) {
                Vec3d velocity = nearbyEntity.getPos().relativize(new Vec3d(rootVec.x + v, rootVec.y + v, rootVec.z + v)).normalize().multiply(multiply);
