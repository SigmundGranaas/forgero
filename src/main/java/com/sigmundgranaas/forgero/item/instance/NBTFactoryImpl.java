...
public class NBTFactoryImpl implements NBTFactory {
    ...
    ForgeroToolPart createToolPartFromNBT(@NotNull NbtCompound compound) {
        String primaryMaterialString = compound.getString(ForgeroToolPartItem.PRIMARY_MATERIAL_IDENTIFIER);
        ...
        String secondaryMaterialString = compound.getString(ForgeroToolPartItem.SECONDARY_MATERIAL_IDENTIFIER);
    ...
    ForgeroTool createToolFromNBT(@NotNull ForgeroToolItem baseTool, @NotNull NbtCompound compound) {
        ...
        ToolPartHead head = (ToolPartHead) createToolPartFromNBT(compound.getCompound(ForgeroToolPartItem.HEAD_IDENTIFIER));
        ToolPartHandle handle = (ToolPartHandle) createToolPartFromNBT(compound.getCompound(ForgeroToolPartItem.HANDLE_IDENTIFIER));
        Optional<ToolPartBinding> binding = Optional.empty();
        if (compound.contains(ForgeroToolPartItem.BINDING_IDENTIFIER)) {
            binding = Optional.of((ToolPartBinding) createToolPartFromNBT(compound.getCompound(ForgeroToolPartItem.BINDING_IDENTIFIER)));
        }
        ForgeroTool tool;
