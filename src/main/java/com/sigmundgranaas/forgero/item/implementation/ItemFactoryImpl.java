...
public class ItemFactoryImpl implements ItemFactory {
    ...
    public Item createTool(ForgeroTool tool) {
        ...
        return switch (tool.getToolType()) {
            case PICKAXE -> new ForgeroPickaxeItem(new SimpleToolMaterialAdapter(tool.getMaterial()), 1, tool.getAttackSpeed(target), new Item.Settings().group(ItemGroup.TOOLS), tool);
            case SHOVEL -> new ShovelItem(new SimpleToolMaterialAdapter(tool.getMaterial()), 1.5f, tool.getAttackSpeed(target), new Item.Settings().group(ItemGroup.TOOLS), tool);
            case AXE -> new ForgeroAxeItem(new SimpleToolMaterialAdapter(tool.getMaterial()), 1, tool.getAttackSpeed(target), new Item.Settings().group(ItemGroup.TOOLS), tool);
            case SWORD -> null;
        };
    }
