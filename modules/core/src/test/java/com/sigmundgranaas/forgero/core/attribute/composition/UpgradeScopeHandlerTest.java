...
/**
 * Tests for {@link UpgradeScopeHandler}.
 *
 * <p>Verifies that upgrade scope attributes only apply when the component
 * is in an upgrade slot.</p>
 */
@DisplayName("UpgradeScopeHandler Tests")
class UpgradeScopeHandlerTest {

	private UpgradeScopeHandler handler;

	@BeforeEach
	void setUp() {
		handler = UpgradeScopeHandler.INSTANCE;
	}

	@Test
	@DisplayName("Handler has correct scope ID")
	void scopeIdIsUpgrade() {
		assertEquals(AttributeScope.UPGRADE, handler.scopeId());
	}
	...
	class UpgradeSourceTests {
		...
		void includesUpgradeAttributes() {
			...
			Map<String, List<Attribute>> sources = new HashMap<>();
			sources.put(UpgradeScopeHandler.UPGRADE_SOURCE_PREFIX + "gem_slot", List.of(attr));
		...
		void excludesNonUpgradeAttributes() {
			...
			Map<String, List<Attribute>> sources = new HashMap<>();
			sources.put(UpgradeScopeHandler.UPGRADE_SOURCE_PREFIX + "socket", List.of(upgradeAttr));
		...
		void includesMultipleUpgradeSources() {
			...
			Map<String, List<Attribute>> sources = new HashMap<>();
			sources.put(UpgradeScopeHandler.UPGRADE_SOURCE_PREFIX + "gem", List.of(gemAttr));
			sources.put(UpgradeScopeHandler.UPGRADE_SOURCE_PREFIX + "reinforcement", List.of(reinforcementAttr));
	...
	class ResolvedPropertiesTests {
		...
		void resolvedHasNoScope() {
			SimpleAttribute attr = new SimpleAttribute(
					...
					AdditionOperator.getInstance(),
					0,
					Optional.of(AttributeScope.UPGRADE),
					Optional.empty()
			);

			Map<String, List<Attribute>> sources = new HashMap<>();
			sources.put(UpgradeScopeHandler.UPGRADE_SOURCE_PREFIX + "gem", List.of(attr));
	...
	class RealWorldTests {
		...
		void gemAsMaterialDoesNotApply() {
