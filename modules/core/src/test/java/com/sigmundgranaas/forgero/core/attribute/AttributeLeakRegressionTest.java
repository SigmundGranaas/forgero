...
/**
 * Core-level tests for attribute calculation MECHANISMS.
 *
 * These tests verify that the attribute engine works correctly:
 * - Attributes aggregate from parts to tools
 * - Conditions filter attributes properly
 * - Components are isolated from each other
 *
 * NOTE: These tests do NOT verify specific configuration values (like "iron = 250 durability").
 * Configuration validation belongs in the Minecraft-level GameTests where the full
 * data pipeline is available.
 *
 * These tests focus on the ENGINE, not the DATA.
 */
@DisplayName("Attribute Engine Mechanism Tests")
class AttributeLeakRegressionTest extends ForgeroTest {
