...
/**
 * Tests for {@link PartCompositeScopeHandler}.
 *
 ...
 * </ul>
 */
@DisplayName("PartCompositeScopeHandler Tests")
class PartCompositeScopeHandlerTest {

	private PartCompositeScopeHandler handler;

	@BeforeEach
	void setUp() {
		handler = PartCompositeScopeHandler.INSTANCE;
	}

	@Test
	@DisplayName("Handler has correct scope ID")
	void scopeIdIsPartComposite() {
		assertEquals(AttributeScope.PART_COMPOSITE, handler.scopeId());
	}
	...
	class ComposedAttributePropertiesTests {
		...
		@Test
		@DisplayName("Composed attributes have no scope")
		void composedHasNoScope() {
			SimpleAttribute base = new SimpleAttribute(
					Optional.empty(),
					DefaultAttributes.ATTACK_DAMAGE,
					4f,
			...
			assertTrue(result.get(0).scope().isEmpty(),
					"Composed attribute should have no scope (already resolved)");
		}
