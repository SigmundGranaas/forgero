...
public class ModelCodecs {
	...
	public static final Codec<ModelDTO> MODEL_DTO_CODEC_DISPATCHER = RecordCodecBuilder.create(instance -> instance.group(
			...
			Codec.list(MountPointDTO.CODEC).optionalFieldOf("mount_points").forGetter(ModelDTO::getMountPoints),
	...
	).apply(instance, (id, type, layers, slots, mountPoints, texture, textures, target, context, parent, display) -> new ModelDTO(id.orElse(null), type, layers.orElse(null), slots.orElse(null), mountPoints.orElse(null), texture.orElse(null), textures.orElse(null), target.orElse(null), context.orElse(null), parent.orElse(null), display.orElse(null))));
}
