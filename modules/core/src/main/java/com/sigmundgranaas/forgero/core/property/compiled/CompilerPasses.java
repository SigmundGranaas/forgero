 ...
 * {@link CompilerPass}. The game modules register their passes here at load time, exactly
...
public final class CompilerPasses {
	private static final Map<OpenIdentifier, Supplier<? extends CompilerPass<? extends java.util.List<?>>>> PASSES =
	 ...
	 */
	public static <P> void register(ResolutionKey<java.util.List<P>> key, Supplier<? extends CompilerPass<java.util.List<P>>> engine) {
		PASSES.put(key.id(), engine);
	}

	public static Map<OpenIdentifier, Supplier<? extends CompilerPass<? extends java.util.List<?>>>> registered() {
		return PASSES;
	}
}
