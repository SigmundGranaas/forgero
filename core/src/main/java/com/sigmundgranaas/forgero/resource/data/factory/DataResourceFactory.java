...
public abstract class DataResourceFactory<T extends ForgeroDataResource, R extends ForgeroResource<T>> implements ForgeroResourceFactory<R, T> {
    ...
    public DataResourceFactory(Collection<T> pojos) {
        this.pojos = pojos.stream().collect(Collectors.toMap(ForgeroDataResource::getName, pojo -> pojo, (current, next) -> resolveConflict(current, next, ForgeroDataResource.DEFAULT_DEPENDENCIES_SET)));
    ...
    protected boolean validatePojo(T pojo) {
        if (!resolveDependencies(pojo)) {
            if (pojo.required) {
                Forgero.LOGGER.error("Unable to create resource {} of type {}, because of missing dependencies: {}", pojo.name, pojo.resourceType, pojo.dependencies);
            }
            return false;
        }
